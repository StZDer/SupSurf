...
@section('content')
    <div class="navbar-point mb-3">
        <div class="navbar-point-title">Редактирование товара</div>
    </div>
    <form method="POST" action="{{ route('product.update', $product->id) }}">
        ...
        <div class="container enter">
            ...
            <label for="price" class="col-12 col-form-label text-center">Точка</label>
            <div class="location">
                <select class="form-select" name="point_id" id="point_id">
                    @foreach ($points as $point)
                        <option @if ($point->id == $product->point_id) selected @endif value="{{ $point->id }}">
                            {{ $point->name }}</option>
                    @endforeach
                </select>
            </div>
